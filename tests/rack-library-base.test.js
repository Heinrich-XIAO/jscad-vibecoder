...
test("rack library includes backing bar", () => {
  loadRackLibrary();
  ...
  const geom = rack.getModel();
  const geometries = Array.isArray(geom) ? geom : [geom];
  const minY = Math.min(
    ...geometries.map((item) => measureBoundingBox(item)[0][1])
  );
  const dedendum = module + clearance;

  expect(minY).toBeLessThan(-dedendum - backHeight / 2);
});
